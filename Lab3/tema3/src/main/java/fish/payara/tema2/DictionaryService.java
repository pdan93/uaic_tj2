...
public class DictionaryService {
    ...
    private static DictionaryService single_instance = null;
    ...
    public static DictionaryService getInstance() {
        if (single_instance == null)
            single_instance = new DictionaryService();
    ...
    public String findWord(String word, String language) {
        if (language != null) {
            ...
            if (dict != null) {
                return dict.getWordDefinition(word);
            }
        } else {
            for (int i=0; i<dictionaries.size(); i++) {
                if (dictionaries.get(i).getWordDefinition(word) != null)
                    return dictionaries.get(i).getWordDefinition(word);
            }
        }
